...
class ClawChatViewProvider {
        ...
        for (const msg of messages) {
            ...
            if (msg.role === "user" && "text" in firstBlock) {
                ...
                const text = msg.content
                    ...
                    .map(b => b.text).join("\n");
                if (text)
                    this.webviewView?.webview.postMessage({ type: "addMessage", role: "assistant", content: text });
            }
        }
    }
    ...
    async runAgenticLoop(prompt) {
        ...
        await this.clawProcess.run(fullPrompt, cwd, (evt) => {
            ...
            switch (evt.type) {
                ...
                case "tool_start": {
                    ...
                    const label = describeToolAction(evt.toolName || "", evt.toolInput ? (() => {
                        try {
                            return JSON.parse(evt.toolInput || "{}");
                        }
                        catch {
                            return {};
                        }
...
<script>${(0, webviewScript_1.getWebviewScript)()}</script>
</body>
</html>`;
    }
}
exports.ClawChatViewProvider = ClawChatViewProvider;
...
function describeToolAction(name, input) {
    ...
    switch (key) {
        ...
        case "glob": return `Searching ${input.pattern || "files"}`;
        case "grep": return `Grepping for ${input.pattern?.slice(0, 40) || "pattern"}`;
        case "listdirectory":
        ...
        case "webfetch": return `Fetching ${input.url?.replace(/^https?:\/\//, "").slice(0, 50) || "URL"}`;
