...
exports.updateProjectContext = updateProjectContext;
exports.getProjectContextPrompt = getProjectContextPrompt;
 ...
 */
function scanTree(dir, prefix = "", depth = 0, maxDepth = 3) {
    if (depth > maxDepth)
        return [];
 ...
 */
function updateProjectContext(workspaceRoot, action, // "created" | "edited" | "deleted"
 ...
 */
function readReadme(workspaceRoot) {
 ...
 */
function getProjectContextPrompt(workspaceRoot) {
    const tree = scanTree(workspaceRoot);
    if (tree.length === 0)
        return "";
    let result = "\n\n## Workspace Structure\n\n```\n" + tree.join("\n") + "\n```";
...
function guessDescription(name, ext) {
    const map = {
